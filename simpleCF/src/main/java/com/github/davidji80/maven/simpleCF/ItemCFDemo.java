package com.github.davidji80.maven.simpleCF;
...
public class ItemCFDemo {
    ...
    //电影相似度矩阵
    private static double[][] movieABMatrix=new double[7][7];
    //待推荐电影相似度列表
    ...
    private static void calcRecommendMovie(){
        ...
        */
        ...
        for (int i=0;i<targetUserCommentedMovies.size();i++){
            if (targetUserCommentedMovies.get(i)==0){
                for (int j=1;j<targetUserCommentedMovies.size();j++){
                    if (targetUserCommentedMovies.get(j)==1){
                        movieSimilaritys.put(i,movieABMatrix[i][j]);
                    }
                }
            }
        }
    ...
    private static void calcAllMovieSimilaritys(){
        ...
        //转换电影相似度矩阵
        Iterator map1it=movieABSimilaritys.entrySet().iterator();
        while(map1it.hasNext()) {
            Map.Entry<String,Double> entry=(Map.Entry<String,Double>) map1it.next();
            int i=Integer.parseInt(entry.getKey().substring(0,1));
            int j=Integer.parseInt(entry.getKey().substring(1,2));
            movieABMatrix[i][j]=entry.getValue();
            movieABMatrix[j][i]=entry.getValue();
        }
    }
